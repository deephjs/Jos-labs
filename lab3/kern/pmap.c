...
page_init(void)
{
	...
	extern char end[];
	pages[1].pp_link = 0;
	struct Page* page_start = pa2page((physaddr_t)IOPHYSMEM);
	struct Page* page_end = pa2page((physaddr_t)(end-KERNBASE+PGSIZE+npages*sizeof(struct Page)));
	page_start = page_start-1;
	page_end = page_end + 1;
	page_end->pp_link = page_start;*/
...
{
	// Fill this function
	if (n <= 0 || !check_continuous(pp, n))
		return -1;
	struct Page *temp;
	temp = pp;
	int i;
	if (n != 1)
		temp = (temp+n-2)->pp_link;
...
{	
	...
	if (old_n > new_n) {
		...
		struct Page *result = page_alloc_npages(0, new_n);
...
page_insert(pde_t *pgdir, struct Page *pp, void *va, int perm)
{
	...
	// Fill this function in
	/*pte_t **pte = NULL;
	struct Page* old = page_lookup(pgdir, va, pte);
	if (old == NULL){
		*pte = pgdir_walk(pgdir,va,1);
		if (*pte == NULL)
			return -E_NO_MEM;
	...
	**pte = page2pa(pp)|perm|PTE_P;
	...
	if (pte == NULL)
		return -E_NO_MEM;
	if (*pte != (pte_t)NULL && pa2page(PTE_ADDR(*pte)) != pp)
		page_remove(pgdir, va);
	*pte = page2pa(pp)|perm|PTE_P;
	pp->pp_ref++;
	return 0;*/
