...
#else
	//LAB 4: Your code here
	//not consider that number of lockers at the same time is over unsigned
	unsigned next = atomic_return_and_add(&(lk->next), 1);
	while(next != lk->own){
		//spin
		//it seems that without cprintf, the scheduler will not give the running chance to another cpu
		cprintf("");
		asm volatile("pause");
	}
#endif
