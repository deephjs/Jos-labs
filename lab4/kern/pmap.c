...
page_init(void)
{
	...
	struct Page* mpentrypage = pa2page((physaddr_t)MPENTRY_PADDR);
	mpentrypage->pp_ref = 1;
	mpentrypage++;
	mpentrypage->pp_link = (mpentrypage-2);
	chunk_list = NULL;
}
...
page_alloc_npages_helper(struct Page **head, int alloc_flags, int n)
{
	...
	struct Page **fakeHead = NULL;

	while(plow && (tmpn > 1)){
		if(plow->pp_link == NULL) return NULL;
	...
	if(tmpn != 1) return NULL;	//out of memory

	if(phigh != *head){
		fakeHead = &(phigh->pp_link);
		phigh = phigh->pp_link;
	}else{
		fakeHead = head;
	}
	...
	*fakeHead = phigh->pp_link;
	phigh->pp_link = NULL;
	while(*fakeHead != plow){
		ptmp = (*fakeHead)->pp_link;
		(*fakeHead)->pp_link = phigh;
		phigh = *fakeHead;
		*fakeHead = ptmp;
	}
...
page_free_npages(struct Page *pp, int n)
{
	...
	int i;
	for(i= 1; i< n; ++i){
		if(!phigh->pp_link) return -1;
		if(page2pa(phigh->pp_link)-page2pa(phigh) != PGSIZE) return -1;
		phigh = phigh->pp_link;
	}
	...
	struct Page **fake_head = &chunk_list;
	if(pcon != chunk_list)
		fake_head = &(pcon->pp_link);
	while((*fake_head) != phigh){
		plow = pp->pp_link;
		pp->pp_link = *fake_head;
		*fake_head = pp;
		pp = plow;
	}
	return 0;	
...
page_realloc_npages_helper(struct Page **head, struct Page *pp, int old_n, int new_n){
	...
	if(ele_num < new_n - old_n) return 0;
	ele_num = ele_num + old_n - new_n - 1;
	while(ele_num > 0){
		phigh = phigh->pp_link;
		ele_num--;
	}
	if((ele_num == 0) && (page2pa(phigh->pp_link) - page2pa(pp) != (new_n-old_n)*PGSIZE)) return 0;
	if((ele_num == -1) && (page2pa(phigh) - page2pa(pp) != (new_n-old_n)*PGSIZE)) return 0;
